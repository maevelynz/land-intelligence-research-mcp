...
flowchart TD
    ...
    subgraph DET["deterministic Python/DuckDB (server.py)"]
        direction TB
        M["MCP typed interface<br/>(function signature, argument types)"] --> A["analytical definition<br/>(PARCEL_FEATURE_SQL, aggregation, decile logic)"]
        A --> R["structured JSON result"]
    end
    S --> M
...
### B — Optionality score distribution

Running this formula over all 3,200 checked-in parcels produces the following
distribution:

![Distribution of optionality_signal across all 3,200 synthetic parcels, with mean and median reference lines](assets/optionality_distribution.png)

The shape here is a joint artifact of the chosen weights/decay constants above and
the synthetic input distributions in `scripts/generate_dummy_data.py` (e.g. acreage
is log-normal, flood risk and wetland share are Beta-distributed) — it is not a
market distribution and should not be read as one. Generated by
`scripts/generate_visualizations.py::make_distribution_chart`, which calls
`parcel_scores()` directly (see [Regenerating these figures](#regenerating-these-figures)).

## The five MCP tools

```mermaid
flowchart LR
    PS[("parcel_scores()<br/>PARCEL_FEATURE_SQL")]
    LRQ["list_research_questions"]
    CC["compare_counties"]
    RLO["rank_land_optionality"]
    EPS["explain_parcel_score"]
    EOS["evaluate_optionality_signal"]
    DO[("development_outcomes")]
...
`list_research_questions` is the only tool that never touches `parcel_scores()` —
it returns a static list. The other four all read from the same single definition
of `optionality_signal`; only `evaluate_optionality_signal` also joins in
`development_outcomes`, the one table none of the other tools ever reference.
...
![Two-panel county comparison: left panel shows average optionality_signal by county, right panel shows count of parcels scoring 60 or above, both sorted by average signal](assets/county_comparison.png)

This is `compare_counties(limit=20)`'s actual output, rendered directly (the values
match the [worked `compare_counties` example](#end-to-end-example) below for the top
three rows). The two panels exist side by side, sharing county order, specifically to
make the tool's own stated limitation visible: Culpeper County has a higher average
signal than Clarke County (40.6 vs. 40.4) but only 1 parcel clears the ≥60 threshold
versus Clarke's 24 — the mean alone would hide that concentration difference.
Generated by `scripts/generate_visualizations.py::make_county_comparison_chart`,
which calls the real `compare_counties()` tool function.
...
The chart below does what `explain_parcel_score`'s own limitation says it doesn't:
it reconstructs each of the ten weighted terms individually for one parcel, so the
point contribution of each feature is visible directly rather than left for the
reader to compute from the formula.

![Formula decomposition of optionality_signal for one representative parcel, showing each of the ten weighted terms sorted by point contribution](assets/score_anatomy.png)

**Parcel selection is deterministic, not hand-picked**: the visualization script
selects the parcel whose `optionality_signal` is closest to the dataset median
(ties broken by ascending `parcel_id`), so re-running the generator against
unchanged data always reproduces the same parcel and chart.

**Numerical self-check**: the ten component values plotted here are recomputed in
`scripts/generate_visualizations.py::_score_components()` from constants copied
verbatim from `PARCEL_FEATURE_SQL` above — this is the one place in the
visualization pipeline that duplicates production logic rather than calling it
directly, because no tool exposes a per-term breakdown. To guard against that
duplication silently drifting from `server.py`, the script asserts the sum of the
ten reconstructed components equals the real `optionality_signal` returned by
`parcel_scores()` for that parcel, within a `1e-6` tolerance, and **raises and
aborts generation** if they disagree. A deliberately-broken constant (e.g.
changing the substation weight from `0.22` to `0.99`) reproducibly fails this
check rather than silently rendering a wrong chart.

This is a **formula decomposition**, not a feature-importance or causal-effect
measure — the same caveat `explain_parcel_score` itself states applies here too.
...
![Two-panel decile validation chart: left panel is average optionality_signal by decile from the deterministic formula, right panel is observed_conversion_5yr rate by decile from the independent synthetic data-generating process](assets/decile_validation.png)

This is `evaluate_optionality_signal()`'s actual output, rendered directly. The two
panels are deliberately **not** combined into one dual-axis chart: the left panel is
`optionality_signal` (deterministic, computed in `server.py` at query time) and the
right panel is `observed_conversion_5yr` (a Bernoulli draw from a separate logistic
model, computed once in `scripts/generate_dummy_data.py` and stored on disk) — two
independent quantities that happen to trend together, not two views of the same
number. The top-vs-bottom lift annotated on the figure is the same
`top_vs_bottom_lift` value the tool itself returns. As with every other figure in
this document: **synthetic validation only, not evidence of real-world predictive
performance.**
...
The four PNGs embedded in this document and in `README.md` (`docs/assets/*.png`) are
generated by `scripts/generate_visualizations.py`, which is deterministic: given the
checked-in CSVs in `data/`, it always produces byte-identical output. It is not run as
part of the test suite or CI — regenerate it manually after a change to `data/` or to
`PARCEL_FEATURE_SQL`:
...
The script imports and calls the real functions in `server.py` directly
(`parcel_scores`, `compare_counties`, `evaluate_optionality_signal`) rather than
re-deriving their SQL or aggregation logic, so three of the four charts (B, C, F)
cannot silently drift from what the MCP tools actually return. The fourth (D, score
anatomy) necessarily reconstructs the ten weighted terms individually, since no tool
exposes a per-term breakdown; that reconstruction is guarded by a hard numerical
self-check against the real `optionality_signal` (see [D — Score
anatomy](#d--score-anatomy) above), so a future change to the score formula that isn't
mirrored in the visualization script fails the regeneration run instead of quietly
producing a wrong chart.

`matplotlib` is an optional `viz` dependency (`pyproject.toml`), not a runtime
dependency of the MCP server itself — installing or running `land-intelligence-mcp`
normally never pulls it in.
